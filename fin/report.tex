...
\end{equation}
where the adjoint spinor is defined by $\bar{\psi} = \psi^\dagger \gamma^0$.
...
Moreover, the vector current is equivalent to the electromagnetic four-current $J^\mu$;
thus, its conservation is equivalent to the conservation of electric charge
\begin{equation}
...
and are defined as having positive and negative chirality respectively.
...
\end{equation}
where $\phi$ and $\bm{A}$ are the electric scalar potential and magnetic vector potential
respectively, $\bm{\sigma}$ is the Pauli vector, and $\mathbb{I}_2$
is the 2$\times$2 identity matrix. By 
using the definition of 4-momentum,\footnote{$P_{\mu} = (\frac{1}{c}E, -\bm{p})$.} we have
...
\end{equation}
where $C$ is the boundary of the surface $S$ in parameter space. The last integral
is the integral of the curvature form of a vector bundle on a
smooth manifold and by using the Chern-Weil theory
we have that
\begin{equation}
  \phi_n = 2\pi c_1,
\end{equation}
...
\end{equation}
where $\varepsilon_0$ and $\bm{k}^w$ are the energy and crystal momentum at the 
...
\begin{equation}
  A_\mu = \braket{u_{-k}^\dagger|\partial_\mu u_{-k}^\dagger} = \braket{\partial_\mu u_{-k}| u_{-k}},
\end{equation}
and the Berry curvature is antisymmetric
\begin{equation}
\begin{split}
\Omega^{\bm{k}}_{\mu\nu} &= \partial_\mu \braket{\partial_\nu u_{-k}| u_{-k}} - \partial_\nu 
  \braket{\partial_\mu u_{-k}| u_{-k}} \\
            & = \braket{\partial_\nu u_{-k}| \partial_\mu u_{-k}} -
          \braket{\partial_\mu u_{-k}| \partial_\nu u_{-k}} = -\Omega^{-\bm{k}}_{\mu\nu}.
\end{split}
\end{equation}
...
a WSM with 24 Weyl points at an intermediate $U \sim$~\qty{1.5}{\electronvolt},
a Mott insulator at $U >$~\qty{2}{\electronvolt}.
...
WSM candidates as the large hall conductivity of Nd$_{2}$Ir$_{2}$O$_{7}$ and its low 
...
and conserved during photoemission ARPES is a useful probe of crystal momenta
in 2D materials (the momentum of a UV photon is negligible). If the material workfunction $\phi$
...
polymorph subsequently predicting a low-temperature WSM phase~\cite{Cd3As2_abin}.
...
symmetries violated by quantum processes. Because of Noether's first theorem
...
producing the Adler-Bell-Jackiw (ABJ) anomaly~\cite{Adler,Bell-Jackiw}. As we shall see, 
the ABJ anomaly (known as the chiral anomaly outside of particle physics) can be 
...
\begin{equation}
  ...
  \exp{\left[ \frac{i}{\hbar} \int{d^4x\, -\frac{1}{4\mu_0 c}F_{\mu\nu}F^{\mu\nu} +
...
\begin{equation}
  Z[A] = \int{\mathcal{D}\psi\mathcal{D}\bar{\psi}\,\mathcal{J}[\vartheta]
    \exp{ \left[\frac{i}{\hbar} \int{d^4x\, \mathcal{L}_{\mathrm{QED}} +
          \vartheta(x)(\partial_\mu j^\mu_5 - 2imc\bar{\psi}\gamma^5\psi) }  \right] } }.
\end{equation}
Thus, to first-order in $\vartheta(x)$ we have
\begin{equation}
  Z[A] = \int{\mathcal{D}\psi\mathcal{D}\bar{\psi}\, \left\{1 + \frac{i}{\hbar} \int{d^4x\,
     \vartheta(x)(\partial_\mu j^\mu_5 - 2imc\bar{\psi}\gamma^5\psi -\mathcal{A})} \right\}
   \exp{\left[ \frac{i}{\hbar} \int{d^4x\, \mathcal{L}_{\mathrm{QED}} } \right]}}.
\end{equation}
...
and momentum are forbidden. Subsequently, we introduce the $n^\text{th}$
...
Using the semi-classical phenomenological equations of
...
to observe identify in TaAs and previously observed LMRs are likely extrinsic.
...
of left and right-handed Weyl fermions is conserved. Although actual Weyl fermions have
not been discovered in nature, electrons behave as if they were Weyl fermions in materials called Weyl
